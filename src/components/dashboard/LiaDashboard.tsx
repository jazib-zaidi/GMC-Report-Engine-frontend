...
import CustomLIAReportBuilder from './LiaReportBuilder';
import PerformanceDashboardSkeleton from './PerformanceDashboardSkeleton';
...
const LiaDashboard = () => {
  ...
  useEffect(() => {
    if (activeTab === 'AI Insight' && bottomRef.current) {
  ...
  const renderInsigth = () => {
    for (const product of liaReportData?.results) {
      const revenue = product.conversions_value;
      ...
      if (revenue > maxRevenue) {
        ...
        topSeller = {
          ...
          revenue,
          cost,
          roas: cost === 0 ? (revenue > 0 ? '∞' : 0) : (revenue / cost) * 100,
        };
    ...
    let message;
    if (topSeller) {
      message = `Top seller across all channels was  <a
                className='text-blue-500 flex text-end'
                href=${metaData?.Link}
                target='_blank'
              >"${topSeller.title}    </a>" with AUD <b>${formatCurrency(
        topSeller.revenue
      )}</b> revenue and <b>${topSeller.roas.toFixed(2)}%</b> ROAS.`;
    } else {
      message = 'No product generated any revenue.';
    }
  ...
  return (
    <div className='flex flex-col gap-6 p-6'>
      <div className='flex justify-between items-center'>
        <Button
          onClick={() => setSelectedAdsAccount(null)}
          variant='outline'
      ...
      {!hasData ? (
        ...
        <>
          <LiaMetrics setExportMatrix={setExportMatrix} />
          <div className='bg-white rounded-lg shadow p-6 flex flex-row items-center gap-x-4'>
            <img
              className='w-40 bg-white border rounded-md  p-2'
              src={
                renderInsigth()?.metaData['Image Link'] ||
                'https://image.pngaaa.com/700/5273700-middle.png'
              }
              alt=''
            />
            {/* <p>{renderInsigth()?.metaData?.Link}</p> */}
            <div className=''>
              <p
                className='text-lg custom-top-performance-tag'
                dangerouslySetInnerHTML={{ __html: renderInsigth().message }}
              />

              <Button
                onClick={() => setActiveTab('AI Insight')}
                variant='outline'
                className=' mt-4 custom-AI-button text-white'
              >
                <svg
                  width='24'
                  height='24'
          ...
          <div className='flex border-b'>
            ...
            <button
              className={`px-4 py-2 font-medium flex gap-x-1 ${
              ...
              onClick={() => setActiveTab('AI Insight')}
            >
              <img className='w-6' src='/aiIcon.png' alt='AI Icon' /> AI Insight
            </button>
          </div>
          ...
          <div ref={bottomRef} className=''>
                  ...
                  <div className='overflow-x-auto'>
                    <table className='min-w-full divide-y divide-gray-200'>
                      ...
                      <tbody className='bg-white divide-y divide-gray-200'>
                        ...
                        {liaReportData.data
                          .filter(
                            (channel) => channel?.product_channel !== 'UNKNOWN'
                          )
                          .map((channel) => {
                            return (
                              <tr className='hover:bg-gray-50'>
                                ...
                                <td className='px-6 py-4 whitespace-nowrap text-right text-sm text-gray-500'>
                                  {formatNumber(
                                    roundtoNearest(channel.conversions)
                                  )}
                                </td>
                                <td className='px-6 py-4 whitespace-nowrap text-right text-sm text-gray-500'>
                                ...
                                <td className='px-6 py-4 whitespace-nowrap text-right text-sm text-gray-500'>
                                  {formatNumber(
                                    (
                                      channel.conversions_value / channel.cost
                                    ).toFixed(2) * 100
                                  )}
                                  %
                                </td>
                                <td className='px-6 py-4 whitespace-nowrap text-center text-sm font-medium'>
                                  <button className='text-[#0077e0] hover:text-[#0077e0] cursor-pointer whitespace-nowrap !rounded-button'>
                                    <Link to={`/${channel.product_channel}`}>
            ...
            {activeTab === 'AI Insight' && <AiInsight />}
          </div>
        </>
      )}
      {/* <CustomLIAReportBuilder /> */}
    </div>
  );
};

export default LiaDashboard;
